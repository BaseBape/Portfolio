...
| 動作名 | 説明 | リンク |
|--------|------|--------|
| Notion 同期 | `main` ブランチへの push 時、**変更のあった「言語名/動作名」のみ** Notion のデータベースに同期。`.github/`、`scripts/`、`README.md` のみの変更では同期しない。 | [.github/workflows/notion-sync.yml](.github/workflows/notion-sync.yml) ・ [scripts/notion_sync.py](scripts/notion_sync.py)
